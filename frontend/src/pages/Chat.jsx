...
const suggestions=[['Elaborar um ETP','Crie um ETP para contratação de '],['Criar um despacho','Elabore uma minuta de despacho sobre '],['Revisar documento','Revise os documentos anexados e indique melhorias.'],['Preparar um TR','Crie um Termo de Referência com base nos anexos.']]
...
export default function Chat(){
 ...
 useEffect(()=>{if(id?.startsWith('local-')){const local=getLocalChat(id);if(local){setConversation(local);setMessages(local.messages||[])}else nav('/');return}if(id)api.get(`/conversations/${id}`).then(r=>{setConversation(r.data);setMessages(r.data.messages)}).catch(()=>nav('/'));else{setConversation(null);setMessages([])}},[id])
 ...
 const send=async e=>{
  ...
  try{
   ...
   try{
    const{data}=await api.post(`/conversations/${conv.id}/messages`,{content:prompt,attachment_ids:attachmentIds})
    const updated=(await api.get(`/conversations/${conv.id}`)).data
    setConversation(updated);setMessages(old=>[...old,data.assistant])
